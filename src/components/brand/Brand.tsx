import Image from "next/image";
import { site } from "@/data/site";
import styles from "./Brand.module.css";

type BrandProps = {
  /** Logo real (public/images/cookies/logo-sweet-cookies.*) o null. */
  logoSrc: string | null;
  size?: "sm" | "md" | "lg";
  /** true cuando el contenedor ya tiene nombre accesible (ej. link con aria-label). */
  decorative?: boolean;
  className?: string;
};

/**
 * Marca de Sweet Cookies: el LOGO REAL tal cual (sin redibujar ni reinterpretar).
 * Se usa solo como identificador; no define tipografía ni estilo del sitio.
 * Solo si el archivo faltara se muestra el nombre en texto.
 */
export function Brand({ logoSrc, size = "md", decorative = false, className }: BrandProps) {
  const classes = [styles.brand, styles[size], className].filter(Boolean).join(" ");
  if (logoSrc) {
    return (
      <span className={`${classes} ${styles.logo}`}>
        <Image
          src={logoSrc}
          alt={decorative ? "" : `${site.name} — logo`}
          width={512}
          height={512}
          sizes="(min-width: 900px) 120px, 96px"
          priority={size === "md"}
        />
      </span>
    );
  }
  return (
    <span className={`${classes} ${styles.wordmark}`} aria-hidden={decorative || undefined}>
      {site.name}
    </span>
  );
}
